...
LaTeX での有名な例では、内積を書く時に $(A, B)$ と表示するためにそのまま `(A, B)` と打つのではなく `\ip{A}{B}` といったマクロを定義するということがあります。
このようにすると、例えば内積をやはり $\langle A, B \rangle$ と表現したいなどとなった時にマクロを変更するだけでよくなります。
