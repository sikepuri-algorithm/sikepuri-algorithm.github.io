...
<ViewSource path="/variable/sound-velocity-revised.ipynb" />

このようにすれば、例えば 10℃ ではなく 20℃ のときの音速を求めたくなったときに次のように `t = 20` とするだけでよくなり、より簡単にプログラムを変更することができます。

<ViewSource path="/variable/sound-velocity-revised-20degree.ipynb" />
...
<Answer>
  <ViewSource path="/variable/to-fahrenheit.ipynb" />
</Answer>
...
3 行目では、`x + y` の値が評価されてその結果が `y` に代入されます。そのため、$2+3=5$ が `y` に代入され `5` が出力されます。
  ...
  <ViewSource path="/variable/substitution-practice.ipynb"/>
</Answer>
