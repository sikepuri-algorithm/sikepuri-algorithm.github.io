...
一応できましたが、このようにつくると関数値を求めるときに毎回 $f(x)$ の定義を見て書かなければなりません。今回のように、単純な関数では良いですがもっと複雑な関数では大変です。このようなときに、Pythonでは関数を使用できます。
...
:::caution
Pythonでは、きちんとインデント（字下げ）をしないとエラーが発生してしまいます。
